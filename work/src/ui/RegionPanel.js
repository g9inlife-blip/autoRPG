(() => {
  ...
  function sync(){
    ...
    updateContext();window.render?.();return true;
  }
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.getElementById('regionSelect')?.addEventListener('change',()=>{
      if(runActive()){lockMessage('던전 진행 중에는 지역/던전을 변경할 수 없습니다.');return;}
      sync();
    });
    document.getElementById('dungeonSelect')?.addEventListener('change',()=>{updateContext();window.render?.();});
  });
