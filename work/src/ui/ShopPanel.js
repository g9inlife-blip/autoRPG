(() => {
  ...
  function renderShop(){
    ...
    const invHtml=getInventory().map(item=>{const price=Math.floor((SHOP_PRICES[item.baseId]||SHOP_PRICES[item.id]||0)*SELL_RATE),locked=isEquipped(item.instanceId),sellable=price>0;return `<div class="shop-item"><div class="shop-item-main"><b>${formatItem(item)}</b><small>#${item.instanceId} · 공 ${item.attack||0} · 방 ${item.defense||0} · 속 ${item.speed||0}${locked?' · 장착 중':''}</small></div><button type="button" data-sell-instance="${item.instanceId}" ${locked||!sellable?'disabled':''}>매각</button></div>`;}).join('')||'<p class="muted">보유 장비가 없습니다.</p>';
