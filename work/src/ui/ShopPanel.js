(() => {
  ...
  function runLootTest(count){const out=document.getElementById('lootTestResult');if(!window.LootSystem||!window.SeededRandom){out.textContent='LootSystem 또는 SeededRandom을 불러오지 못했습니다.';return}const level=Math.max(1,Number(document.getElementById('lootTestLevel')?.value)||1),rng=new SeededRandom(Number(document.getElementById('seed')?.value)||12345),rarity={common:0,uncommon:0,rare:0,unique:0,legendary:0},enh={0:0,1:0,2:0};let last=null;for(let i=0;i<count;i++){last=LootSystem.generate({rng,level,force:true});if(last){rarity[last.rarity]=(rarity[last.rarity]||0)+1;enh[last.enhancement]=(enh[last.enhancement]||0)+1;}}const pct=o=>Object.fromEntries(Object.entries(o).map(([k,v])=>[k,(v/count*100).toFixed(2)+'%']));out.textContent=JSON.stringify({level,samples:count,rarity,rarityPercent:pct(rarity),enhancement:enh,enhancementPercent:pct(enh),lastDrop:last},null,2);}
  window.shopRender=renderShop;window.shopInventory=()=>getInventory();window.shopBuy=buyItem;window.shopSell=sellInstance;window.addLootInstance=addLootInstance;window.isEquipmentOwned=id=>getInventory().some(x=>x.instanceId===id||x.baseId===id);window.getEquipmentQuantity=id=>getInventory().filter(x=>x.baseId===id||x.id===id).length;window.getEquipmentInstance=id=>owned.get(id)||null;
  window.addEventListener('DOMContentLoaded',()=>{syncInventory();renderShop();const toolbar=document.querySelector('.toolbar');if(toolbar&&!document.getElementById('lootTestLauncher')){const b=document.createElement('button');b.id='lootTestLauncher';b.type='button';b.textContent='Loot 테스트';b.style.cssText='grid-column:1 / -1;min-height:38px;font-size:12px';b.addEventListener('click',openLootTest);toolbar.appendChild(b);}});
})();
