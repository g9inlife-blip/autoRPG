(() => {
  ...
  function getEncounter(dungeon,area,rng){
    ...
    if(encounter?.enemies?.length)return encounter.enemies.map(m=>window.MonsterXmlLoader?.state?.items?.find(x=>x.dungeonName===dungeon.monsterDungeonName&&x.name===m.name)).filter(Boolean);
