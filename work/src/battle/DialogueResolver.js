class DialogueResolver {
  ...
  resolve({action,actor,party=[],event=null,dungeon=null}){
    const candidates=[];
  ...
  getBondLevel(actor,party){
    if(!actor||!party?.length)return null;
    const bonds=party.filter(p=>p.id!==actor.id).map(p=>actor.bonds?.[p.id]??0);
    if(!bonds.length)return null;
    const value=Math.max(...bonds); return value>=80?'high':value<=20?'low':null;
  }
}
