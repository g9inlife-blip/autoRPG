...
window.syncXmlDungeons = function(){
  ...
  if(proto&&!proto._xmlDungeonPatch){
    const originalGet=proto.getXmlEnemies;proto.getXmlEnemies=function(){
      const name=this.dungeon?.monsterDungeonName||this.dungeon?.name;const target=Number(this.dungeon?.levelMap?.[(Number(this.floor)||1)-1]??this.dungeon?.levelStart??this.floor)||1;let pool=loader.getForDungeon(name,target);if(!pool.length)pool=loader.getForDungeon(name);if(!pool.length)return originalGet.call(this);return pool;
    };
    const originalFight=proto.fight;proto.fight=function(enemies){const result=originalFight.call(this,enemies);const last=this.events?.[this.events.length-1];if(last?.type==='BATTLE_END')last.data.enemies=(enemies||[]).map(e=>({id:e.monsterId||e.id,name:e.name,grade:e.monsterGrade||e.grade,level:e.level||e.monsterLevel||null}));return result;};
